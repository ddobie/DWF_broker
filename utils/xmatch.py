...
def refine_search_gaia(
    ...
    ----------
    out: List of Tuple
        Each tuple contains (objectId, ra, dec, source, plx, plxerr, gmag, angdist).
        If the object is not found in Gaia, source, plx, plxerr, gmag, angdist
        are marked as Unknown. In the case several objects match
        the centroid of the alert, only the closest is returned.
    """
    out = []
...
def cross_match_alerts_raw_gaia(oid: list, ra: list, dec: list, ctlg: str) -> list:
    ...
    out = refine_search_gaia(ra, dec, oid, id_out, source, plx, plxerr, gmag, angDist)
...
def cross_match_gaia(list_idx, list_ra, list_dec, ctlg="vizier:I/345/gaia2"):
    ...
    xmatch_gaia_plx = np.transpose(matches)[4]
    xmatch_gaia_plxerr = np.transpose(matches)[5]
    xmatch_gaia_gmag = np.transpose(matches)[6]
    xmatch_gaia_angdist = np.transpose(matches)[7]
